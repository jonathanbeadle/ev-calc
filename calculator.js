function toggleCustomElectricityCost() {
    ...
    if (costPerKwhSelect.value === 'custom') {
        ...
        costPerKwhInput.value = ''; // Clear previous custom input
    } else {
        costPerKwhInput.style.display = 'none';
        costPerKwhInput.value = costPerKwhSelect.value; // Use selected value
    }
}
...
// Populate diesel vehicle data based on selection
function populateDieselVehicleData() {
    ...
    if (dieselVehicleSelect === 'vauxhall_vivaro_diesel') {
        mpgDieselInput.value = 37.6; // Example MPG for Vauxhall Vivaro Diesel
        mpgDieselInput.disabled = true; // Disable the input field
    } else if (dieselVehicleSelect === 'ford_transit_diesel') {
        mpgDieselInput.value = 39.8; // Example MPG for Ford Transit Diesel
        mpgDieselInput.disabled = true; // Disable the input field
    } else {
        mpgDieselInput.value = ''; // Reset for custom input
        mpgDieselInput.disabled = false; // Enable the input field for custom input
    }
}

// Populate EV vehicle data based on selection
function populateEVVehicleData() {
    ...
    if (evVehicleSelect === 'vauxhall_vivaro_ev') {
        batterySizeInput.value = 75; // Example battery size for Vauxhall Vivaro EV
        evRangeInput.value = 170; // Example range for Vauxhall Vivaro EV
        batterySizeInput.disabled = true; // Disable the input field
        evRangeInput.disabled = true; // Disable the input field
    } else if (evVehicleSelect === 'ford_transit_ev') {
        batterySizeInput.value = 68; // Example battery size for Ford Transit EV
        evRangeInput.value = 160; // Example range for Ford Transit EV
        batterySizeInput.disabled = true; // Disable the input field
        evRangeInput.disabled = true; // Disable the input field
    } else {
        batterySizeInput.value = ''; // Reset for custom input
        evRangeInput.value = ''; // Reset for custom input
        batterySizeInput.disabled = false; // Enable the input field for custom input
        evRangeInput.disabled = false; // Enable the input field for custom input
    }
}
...
function calculateSavings() {
    ...
    // Create the Diesel Vehicle table
    ...
    // Determine the highlight class for the results
    ...
    // Insert the tables into the summary box
    summaryContent.innerHTML = `
    ...
    // Insert the CO2 table into the CO2 summary box
    co2SummaryContent.innerHTML = `
